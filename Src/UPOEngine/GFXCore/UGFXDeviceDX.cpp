...
namespace UPO
{
	...
	GFXDeviceDX* GFXDeviceDX::Create()
	{
		...
		auto gfxDevice = new GFXDeviceDX(device, immediateContext, featureLevel);
	...
	GFXSwapChain* GFXDeviceDX::CreateSwapChain(const GFXSwapChain_Desc& param)
	{
		GFXSwapChainDX* swapchain = new GFXSwapChainDX(param);
		return swapchain;
	}
	...
	bool GFXSwapChainDX::Init()
	{
		mHWND = (HWND)mDesc.mGameWindow->GetWinHandle();
		mDesc.mGameWindow->GetWinSize(mBackBufferSize);

		ULOG_MESSAGE("creating swapchain % ...", mBackBufferSize);
		...
		swapChainDesc.SampleDesc.Count = mDesc.mSampleCount; /*Max(GEngineConfig()->AsNumber("GFX.MultiSample"), 1.0f)*/;
		...
		if (FAILED(factory->CreateSwapChain(gGFXDX->mDevice, &swapChainDesc, &mSwapchain)))
		{
		...
		result = gGFXDX->mDevice->CreateRenderTargetView(backBuffer, NULL, &backBufferView);
		if (FAILED(result))
		{
	...
	bool GFXSwapChainDX::Resize(const Vec2I& newSize)
	{
		...
		if (FAILED(gGFXDX->mDevice->CreateRenderTargetView(backBuffer, NULL, &backbufferView)))
		{
