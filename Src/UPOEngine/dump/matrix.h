	...
	void Matrix4::MakePerspectiveRH(float fovDegree, float aspect, float zNear, float zFar)
	{
	...
	void Matrix4::MakePerspectiveLH(float fovDegree, float aspect, float zNear, float zFar)
	{
		DirectX::XMMATRIX matrix = DirectX::XMMatrixTranspose(DirectX::XMMatrixPerspectiveFovLH(fovDegree * DEG2RAD, aspect, zNear, zFar));
		
		MemCopy(this, &matrix, sizeof(Matrix4));
	}
