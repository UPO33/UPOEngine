...
namespace UPO
{
	...
	class UAPI GFXRenderer
	{
		...
		GFXTexture2D* mBackBuffer = nullptr;
		GFXTexture2D* mDepthBuffer = nullptr;
		GFXShader* mVSTestColorQuad = nullptr;
		GFXShader* mPSTestColorQuad = nullptr;
	public:

		GFXContext* Context() const { return mContext; }
		GFXDevice* Device() const { return gGFX; }

		void Init(unsigned w, unsigned h, GFXContext* ctx);
		void Release();
