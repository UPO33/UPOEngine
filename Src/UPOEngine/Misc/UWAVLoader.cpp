...
#pragma pack(pop)
	...
	CWavesLoader* CWavesLoader::Get()
	{
		static UPO::TInstance<CWavesLoader>	Instance;
		return Instance;
	}

	WAVERESULT CWavesLoader::LoadWaveFile(const char *szFilename, WAVEID *pWaveID)
	{
	...
	WAVERESULT CWavesLoader::OpenWaveFile(const char *szFilename, WAVEID *pWaveID)
	{
		WAVERESULT wr = WR_OUTOFMEMORY;
		LPWAVEFILEINFO pWaveInfo;
	...
	WAVERESULT CWavesLoader::ReadWaveData(WAVEID WaveID, void *pData, unsigned long ulDataSize, unsigned long *pulBytesWritten)
	{
		LPWAVEFILEINFO pWaveInfo;
	...
	WAVERESULT CWavesLoader::SetWaveDataOffset(WAVEID WaveID, unsigned long ulOffset)
	{
		LPWAVEFILEINFO pWaveInfo;
	...
	WAVERESULT CWavesLoader::GetWaveDataOffset(WAVEID WaveID, unsigned long *pulOffset)
	{
		LPWAVEFILEINFO pWaveInfo;
		WAVERESULT wr = WR_INVALIDPARAM;
	...
	WAVERESULT CWavesLoader::ParseFile(const char *szFilename, LPWAVEFILEINFO pWaveInfo)
	{
	...
	WAVERESULT CWavesLoader::ParseFile(UPO::Stream& stream, LPWAVEFILEINFO pWaveInfo)
	{
		return WAVERESULT::WR_FILEERROR;
		/*
		using namespace UPO;
		...
		MemZero(pWaveInfo, sizeof(WAVEFILEINFO));

			// Read Wave file header
			//fread(&waveFileHeader, 1, sizeof(WAVEFILEHEADER), pWaveInfo->pFile);
			stream.Bytes(&waveFileHeader, sizeof(WAVEFILEHEADER));

			if (!_strnicmp(waveFileHeader.szRIFF, "RIFF", 4) && !_strnicmp(waveFileHeader.szWAVE, "WAVE", 4))
			{
				stream.Bytes(&riffChunk, sizeof(RIFFCHUNK));
				...
				while(!stream.HasError())
				{
					if (!_strnicmp(riffChunk.szChunkName, "fmt ", 4))
					{
						...
						{
							//fread(&waveFmt, 1, riffChunk.ulChunkSize, pWaveInfo->pFile);
							stream.Bytes(&waveFmt, riffChunk.ulChunkSize);
						...
						{
							//fseek(pWaveInfo->pFile, riffChunk.ulChunkSize, SEEK_CUR);
							stream.SetSeek(riffChunk.ulChunkSize);
						}
					}
					else if (!_strnicmp(riffChunk.szChunkName, "data", 4))
					{
	...
	WAVERESULT CWavesLoader::DeleteWaveFile(WAVEID WaveID)
	{
		WAVERESULT wr = WR_OK;
	...
	WAVERESULT CWavesLoader::GetWaveType(WAVEID WaveID, WAVEFILETYPE *wfType)
	{
		if (!IsWaveID(WaveID))
	...
	WAVERESULT CWavesLoader::GetWaveFormatExHeader(WAVEID WaveID, WAVEFORMATEX *wfex)
	{
		if (!IsWaveID(WaveID))
			return WR_INVALIDWAVEID;
	...
	WAVERESULT CWavesLoader::GetWaveFormatExtensibleHeader(WAVEID WaveID, WAVEFORMATEXTENSIBLE *wfext)
	{
		if (!IsWaveID(WaveID))
			return WR_INVALIDWAVEID;
	...
	WAVERESULT CWavesLoader::GetWaveData(WAVEID WaveID, void **lplpAudioData)
	{
		if (!IsWaveID(WaveID))
			return WR_INVALIDWAVEID;
	...
	WAVERESULT CWavesLoader::GetWaveSize(WAVEID WaveID, unsigned long *size)
	{
		if (!IsWaveID(WaveID))
			return WR_INVALIDWAVEID;
	...
	WAVERESULT CWavesLoader::GetWaveFrequency(WAVEID WaveID, unsigned long *pulFrequency)
	{
		WAVERESULT wr = WR_OK;
	...
	WAVERESULT CWavesLoader::GetWaveALBufferFormat(WAVEID WaveID, PFNALGETENUMVALUE pfnGetEnumValue, unsigned long *pulFormat)
	{
		WAVERESULT wr = WR_OK;
	...
	bool CWavesLoader::IsWaveID(WAVEID WaveID)
	{
		bool bReturn = false;
	...
	char *CWavesLoader::GetErrorString(WAVERESULT wr, char *szErrorString, unsigned long nSizeOfErrorString)
	{
		switch (wr)
		{
		case WR_OK:
